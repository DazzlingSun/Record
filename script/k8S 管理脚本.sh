...
# 需要执行 swapoff -a  /etc/fstab 删掉swap相关
#
# Server side:
...
# Client side:
# curl -L http://172.16.100.246:8000/install.sh |  bash -s join --token=6494bc.546a74f666168c88 172.16.100.246:6443

# https://master_ip:31234 访问dashboard

set -x
set -e

HTTP_SERVER=172.16.100.246:8000
...
kube::load_images()
{
    ...
    master_images=(
        kube-apiserver-amd64_v1.8.5
        kube-controller-manager-amd64_v1.8.5
        kube-scheduler-amd64_v1.8.5
        kube-proxy-amd64_v1.8.5
        pause-amd64_3.0
        ...
        kubernetes-dashboard-amd64_1.8.0
    )
...
kube::master_up()
{
    ...
    kubeadm init --kubernetes-version=v1.8.5 --pod-network-cidr=10.96.0.0/12
    ...
    kubectl apply -f http://$HTTP_SERVER/network/kubernetes-dashboard.yml
    kubectl apply -f http://$HTTP_SERVER/network/kubernetes-dashboard-admin.yaml
    ...
    echo kubectl describe secret kubernetes-dashboard-token-j9dpf -n kube-system
}
